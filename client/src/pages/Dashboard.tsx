...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 p-4 md:p-10 space-y-10">
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight">Olá, {user?.name?.split(' ')[0] || 'Empresário'}!</h1>
          <p className="text-slate-500 text-sm font-medium">Veja como estão as movimentações do seu comércio hoje.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white p-8 rounded-[32px] shadow-card border border-slate-100/50">
          <h3 className="text-lg font-black text-slate-900 mb-8">Balanço Financeiro</h3>
          <div className="h-64">
        ...
        <div className="bg-slate-900 p-8 rounded-[32px] shadow-premium flex flex-col justify-between relative overflow-hidden border border-slate-800">
      ...
      <div className="bg-white rounded-[32px] shadow-card border border-slate-100/50 overflow-hidden">
        <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <h3 className="text-lg font-black text-slate-900">Movimentações Recentes</h3>
          <div className="flex items-center gap-2">
            <button 
              onClick={handleExportPDF}
              className="flex items-center gap-2 px-4 py-2 bg-slate-50 border border-slate-200 text-slate-600 rounded-xl text-xs font-black hover:bg-slate-100 transition-all"
            >
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-slate-50/50">
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase">Descrição</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase">Categoria</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase text-center">Status</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase text-right">Valor</th>
                <th className="px-8 py-4 text-[10px] font-black text-slate-400 uppercase text-center">Ações</th>
              </tr>
            </thead>
                ...
                <tr key={t.id} className="hover:bg-slate-50/30 transition-colors group">
                  <td className="px-8 py-5">
                    <div className="flex flex-col">
                      <span className="text-[13px] text-slate-900 font-bold">{t.description}</span>
                  ...
                  <td className={`px-8 py-5 text-[13px] font-black text-right ${t.type === 'INCOME' ? 'text-emerald-600' : 'text-slate-900'}`}>
                    {t.type === 'INCOME' ? '+' : '-'} {formatCurrency(t.amount)}
                  </td>
                  <td className="px-8 py-5 text-center">
                    <div className="flex items-center justify-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => handleEditClick(t)} className="p-2 hover:bg-brand-50 text-slate-400 hover:text-brand-600 rounded-lg transition-all"><Edit2 size={14} /></button>
                      <button onClick={() => handleDelete(t.id)} className="p-2 hover:bg-rose-50 text-slate-400 hover:text-rose-600 rounded-lg transition-all"><Trash2 size={14} /></button>
                    </div>
                  </td>
                </tr>
              ))}
...
const StatCard = ({ title, value, type, label, icon }: any) => {
  ...
  return (
    <div className={`${isBrand ? 'bg-brand-600 shadow-premium' : 'bg-white shadow-card border border-slate-100/50 hover:border-' + styles + '-100'} p-6 rounded-3xl flex flex-col justify-between h-40 transition-all`}>
      <div className="flex justify-between items-start">
        <div className={`p-2.5 ${isBrand ? 'bg-white/20 text-white' : 'bg-' + styles + '-50 text-' + styles + '-600'} rounded-xl`}>{icon}</div>
        <span className={`text-[10px] font-black ${isBrand ? 'text-white bg-white/20' : 'text-' + styles + '-600 bg-' + styles + '-50'} px-2 py-1 rounded-lg uppercase`}>{label}</span>
      </div>
      <div>
        <p className={`${isBrand ? 'text-brand-100' : 'text-slate-400'} text-[11px] font-bold uppercase tracking-wider mb-1`}>{title}</p>
        <p className={`text-2xl font-black ${isBrand ? 'text-white' : 'text-slate-900'}`}>{formatCurrency(value)}</p>
      </div>
    </div>
  );
};
